			...
			<table>
				<thead>
					<tr>
						...
						<th><g:message code="role.ventures.label" default="Ventures" /></th>
				...
				<g:each in="${roleInstanceList}" status="i" var="roleInstance">
					<tr class="${(i % 2) == 0 ? 'even' : 'odd'}">
						...
						<td><g:link action="show" id="${roleInstance.id}">${fieldValue(bean: roleInstance, field: "owner")}</g:link></td>
					
						<td>${fieldValue(bean: roleInstance, field: "name")}</td>
					
						<td>${fieldValue(bean: roleInstance, field: "ventures")}</td>
